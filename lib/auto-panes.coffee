...
module.exports = AtomAutoPanes =
	...
	onNewEditor: (editor) =>
		...
		self = atom.packages.loadedPackages['auto-panes'].mainModule
		...
		if atom.config.get('auto-panes.autoClose')
		...
		if atom.config.get('auto-panes.multiEditors') is 'Open in new pane'
